...
fetch("/handlers/api/workouts/range")
  .then((response) => {
...
function populateChart(workouts) {
  ...
  let workoutDayNames = workouts.map(
    (workout) => dayNames[new Date(workout.day).getDay()]
  );
      ...
      datasets: [
        {
          ...
          data: workouts.map((workout) =>
            workout.exercises.reduce(
              (acc, exercise) => acc + exerciseTotalWeight(exercise),
              0
            )
          ),
          backgroundColor: [
...
function getTotalTimeByExercise(workouts) {
  ...
  workouts.forEach((workout) => {
    workout.exercises.forEach((exercise) => {
      totalTimeByExercise[exercise.name] =
        exercise.duration + (totalTimeByExercise[exercise.name] || 0);
    });
  });
...
function getTotalWeightByExercise(workouts) {
  ...
  workouts.forEach((workout) => {
    workout.exercises
      ...
      .forEach((exercise) => {
        totalWeightByExercise[exercise.name] =
          exerciseTotalWeight(exercise) +
          (totalWeightByExercise[exercise.name] || 0);
      });
  });
